...
public class CloudletSchedulerLIFO extends CloudletSchedulerSpaceShared {
    @Override
    public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
        List<ResCloudlet> lifoQueue = new ArrayList<>();
        Iterator var13 = this.getCloudletWaitingList().iterator();
        while(var13.hasNext()) {
            ResCloudlet rcl = (ResCloudlet) var13.next();
            lifoQueue.add(rcl);
        }
        //Sort by Cloud id in descending order -  Last In Last Out
        Collections.sort(lifoQueue, new Comparator<ResCloudlet>() {
            @Override
            public int compare(ResCloudlet r1, ResCloudlet r2) {
                return  r2.getCloudletId() -  r1.getCloudletId() ;
            }
        });

        this.setCloudletWaitingList(lifoQueue);

...
public class CloudSchedulerPriority extends CloudletSchedulerSpaceShared {
    @Override
    public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
        List<ResCloudlet> prioritizedQueue = new ArrayList<>();
        Iterator var13 = this.getCloudletWaitingList().iterator();
        while(var13.hasNext()) {
            ResCloudlet rcl = (ResCloudlet) var13.next();
            prioritizedQueue.add(rcl);
        }
        //Sort by Cloudlet's priority
        Collections.sort(prioritizedQueue, new Comparator<ResCloudlet>() {
            @Override
            public int compare(ResCloudlet r1, ResCloudlet r2) {
                CloudletWithPriority p1 = (CloudletWithPriority)r1.getCloudlet();
                CloudletWithPriority p2 = (CloudletWithPriority)r2.getCloudlet();
                return   p2.getPriorityLevel().getValue() - p1.getPriorityLevel().getValue();
            }
        });

        this.setCloudletWaitingList(prioritizedQueue);

...
public class CloudletSchedulerSJF extends CloudletSchedulerSpaceShared {
    ...
    public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
        List<ResCloudlet> sjfQueue = new ArrayList<>();
        Iterator var13 = this.getCloudletWaitingList().iterator();
        while(var13.hasNext()) {
            ResCloudlet rcl = (ResCloudlet) var13.next();
            sjfQueue.add(rcl);
        }
        //Sort by Cloudlet length -  Shortest Job First
        Collections.sort(sjfQueue, new Comparator<ResCloudlet>() {
            @Override
            public int compare(ResCloudlet r1, ResCloudlet r2) {
                return  (int)r1.getCloudletLength() -  (int)r2.getCloudletLength();
            }
        });

        this.setCloudletWaitingList(sjfQueue);

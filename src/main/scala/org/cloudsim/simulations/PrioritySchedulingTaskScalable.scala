...
import org.cloudsim.extensions.{CloudSchedulerPriority, CloudletPriorityLevel, CloudletWithPriority}
import scala.collection.JavaConverters._
import scala.collection.{immutable, mutable}

/*
 * A CloudSim Simulation showing how to create scalable cloud simulations.
 * In this simulation, we prioritize tasks based on the priority set for them by the databroker.
 *
 * @author Amna Irfan
 */

// Main class
object PrioritySchedulingTaskScalable extends App with LazyLogging {

    /*
     * Load Typesafe Lightbend Configuration Files
     */
    val defaultConfig: Config = ConfigFactory.parseResources("simulation_priority.conf")
    val fallbackConfig: Config = ConfigFactory.parseResources("simulation_priority.conf")
      .withFallback(PrioritySchedulingTaskScalable.defaultConfig).resolve
     ...
     */
    Log.printLine("Starting Priority Scheduling Task...")
    ...
    try {
        ...
        */
        ...
        @SuppressWarnings(Array.apply("unused")) val datacenter0 = PrioritySchedulingTaskScalable.createDatacenter("Datacenter_0")
        @SuppressWarnings(Array.apply("unused")) val datacenter1 = PrioritySchedulingTaskScalable.createDatacenter("Datacenter_1")
        ...
        */
        ...
        val Some(broker: DatacenterBroker) = PrioritySchedulingTaskScalable.createBroker
        ...
        val vmlist = PrioritySchedulingTaskScalable.createVM(brokerId, total_vms) //creating 20 vms
        ...
        val cloudletList = PrioritySchedulingTaskScalable.createCloudlet(brokerId, total_cloudlets) // creating 40 cloudlets
        ...
        PrioritySchedulingTaskScalable.printCloudletList(newList)
        logger.info("Cloudlet results printed")

        Log.printLine("PrioritySchedulingTaskScalable completed!")
    } catch {
        case e: Exception =>
            e.printStackTrace()
    ...
    def createVM(userId: Int, vms: Int) = {
        ...
        range_vms.foreach((vm_no: Int) => {
            //assign virtual machine the priority scheduler.
            vm(vm_no) = new Vm(vm_no, userId, mips, pesNumber, ram, bw, size, vmm, new CloudSchedulerPriority)
            list += vm.apply(vm_no)
        })
        list
    }
    ...
    def createCloudlet(userId: Int, cloudlets: Int) = {
        ...
        range_cloudlets.foreach((cloudlet_no: Int) => {
            val randomPriority = scala.util.Random.nextInt(4)
            val priorityLevel = randomPriority match {
                case 0 => CloudletPriorityLevel.LOW
                case 1 => CloudletPriorityLevel.NORMAL
                case 2 => CloudletPriorityLevel.IMPORTANT
                case 3 => CloudletPriorityLevel.CRITICAL
            }
            cloudlet(cloudlet_no) = new CloudletWithPriority(cloudlet_no, length, pesNumber, fileSize, outputSize,
                cloudletUtilizationModel, cloudletUtilizationModel, cloudletUtilizationModel, priorityLevel)
     ...
     */
    def printCloudletList(list: util.List[_ <: CloudletWithPriority]): Unit = {
        val indent = "    "
        Log.printLine()
        Log.printLine("========== OUTPUT ==========")
        Log.printLine("Cloudlet ID" + indent + "Priority" + indent + "STATUS" + indent + "Data center ID" + indent + "VM ID" + indent + indent +
          "Time" + indent + "Start Time" + indent + "Finish Time" + indent + "Processing Cost")
        val dft = new DecimalFormat("###.##")

        var totalCost: Double = 0.0D
        list.forEach((cloudlet: CloudletWithPriority) => {
            Log.print(indent + cloudlet.getCloudletId + indent + indent)
            if (cloudlet.getCloudletStatus == Cloudlet.SUCCESS) {
                Log.print(cloudlet.getPriorityLevel()+ indent +  indent  +  indent )
                Log.print("SUCCESS")

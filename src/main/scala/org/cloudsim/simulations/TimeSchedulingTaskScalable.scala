 ...
 * In this simulation, use the cloudsim library's default time shared cloudlet scheduler.
 *
 * @author Amna Irfan
 */

// Main class
object TimeSchedulingTaskScalable extends App with LazyLogging {

  /*
   * Load Typesafe Lightbend Configuration Files
   */
  val defaultConfig: Config = ConfigFactory.parseResources("simulation_time.conf")
  val fallbackConfig: Config = ConfigFactory.parseResources("simulation_time.conf")
    .withFallback(TimeSchedulingTaskScalable.defaultConfig).resolve
  ...
  */
  Log.printLine("Starting Time-Shared Scheduling Task...")
  ...
  try {
    ...
    */
    ...
    @SuppressWarnings(Array.apply("unused")) val datacenter0 = TimeSchedulingTaskScalable.createDatacenter("Datacenter_0")
    @SuppressWarnings(Array.apply("unused")) val datacenter1 = TimeSchedulingTaskScalable.createDatacenter("Datacenter_1")
    ...
    */
    ...
    val Some(broker: DatacenterBroker) = TimeSchedulingTaskScalable.createBroker
    ...
    val vmlist = TimeSchedulingTaskScalable.createVM(brokerId, total_vms) //creating 20 vms
    ...
    val cloudletList = TimeSchedulingTaskScalable.createCloudlet(brokerId, total_cloudlets) // creating 40 cloudlets
    ...
    TimeSchedulingTaskScalable.printCloudletList(newList)
    logger.info("Cloudlet results printed")

    Log.printLine("TimeSchedulingTaskScalable completed!")
  } catch {
    case e: Exception =>
      e.printStackTrace()
  ...
  def createVM(userId: Int, vms: Int) = {
    ...
    range_vms.foreach((vm_no: Int) => {
      //assign virtual machine the Time-shared scheduler.

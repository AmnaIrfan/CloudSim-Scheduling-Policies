...
import org.cloudsim.extensions.{CloudletSchedulerSJF, CloudletWithCost}
 ...
 * In this simulation, we prioritize tasks based on their processing length (shortest job first)
 *
 * @author Amna Irfan
 */

// Main class
object SJFSchedulingTaskScalable extends App with LazyLogging {

  /*
   * Load Typesafe Lightbend Configuration Files
   */
  val defaultConfig: Config = ConfigFactory.parseResources("simulation_sjf.conf")
  val fallbackConfig: Config = ConfigFactory.parseResources("simulation_sjf.conf")
    .withFallback(SJFSchedulingTaskScalable.defaultConfig).resolve
  ...
  */
  Log.printLine("Starting SJF Scheduling Task...")
  ...
  try {
    ...
    */
    ...
    @SuppressWarnings(Array.apply("unused")) val datacenter0 = SJFSchedulingTaskScalable.createDatacenter("Datacenter_0")
    @SuppressWarnings(Array.apply("unused")) val datacenter1 = SJFSchedulingTaskScalable.createDatacenter("Datacenter_1")
    ...
    */
    ...
    val Some(broker: DatacenterBroker) = SJFSchedulingTaskScalable.createBroker
    ...
    val vmlist = SJFSchedulingTaskScalable.createVM(brokerId, total_vms) //creating 20 vms
    ...
    val cloudletList = SJFSchedulingTaskScalable.createCloudlet(brokerId, total_cloudlets) // creating 40 cloudlets
    ...
    SJFSchedulingTaskScalable.printCloudletList(newList)
    logger.info("Cloudlet results printed")

    Log.printLine("SJFSchedulingTaskScalable completed!")
  } catch {
    case e: Exception =>
      e.printStackTrace()
  ...
  def createVM(userId: Int, vms: Int) = {
    ...
    range_vms.foreach((vm_no: Int) => {
      //assign virtual machine the SJF scheduler.
      vm(vm_no) = new Vm(vm_no, userId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerSJF)
      list += vm.apply(vm_no)
    })
    list
  }
  ...
  def createCloudlet(userId: Int, cloudlets: Int) = {
    ...
    range_cloudlets.foreach((cloudlet_no: Int) => {
      ...
      val length = (1 +  scala.util.Random.nextInt(9)) * 1000

      cloudlet(cloudlet_no) = new CloudletWithCost(cloudlet_no, length, 1, fileSize, outputSize,
  ...
  def printCloudletList(list: util.List[_ <: Cloudlet]): Unit = {
    ...
    Log.printLine("Cloudlet ID" + indent + "STATUS" + indent + "PEs" +indent + "Length" + indent + "Data center ID" + indent + "VM ID" + indent + indent +
    ...
    list.forEach((cloudlet: Cloudlet) => {
      ...
      if (cloudlet.getCloudletStatus == Cloudlet.SUCCESS) {
        Log.print("SUCCESS")
        Log.print(indent+cloudlet.getNumberOfPes())
        Log.print(indent+indent+cloudlet.getCloudletLength())

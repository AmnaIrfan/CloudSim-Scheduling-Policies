...
import org.cloudsim.extensions.{CloudletSchedulerLIFO, CloudletWithCost}
 ...
 * In this simulation, we prioritize tasks based on last in first out
 *
 * @author Amna Irfan
 */

// Main class
object LIFOSchedulingTaskScalable extends App with LazyLogging {

  /*
   * Load Typesafe Lightbend Configuration Files
   */
  val defaultConfig: Config = ConfigFactory.parseResources("simulation_lifo.conf")
  val fallbackConfig: Config = ConfigFactory.parseResources("simulation_lifo.conf")
    .withFallback(LIFOSchedulingTaskScalable.defaultConfig).resolve
  ...
  */
  Log.printLine("Starting LIFO Scheduling Task...")
  ...
  try {
    ...
    */
    ...
    @SuppressWarnings(Array.apply("unused")) val datacenter0 = LIFOSchedulingTaskScalable.createDatacenter("Datacenter_0")
    @SuppressWarnings(Array.apply("unused")) val datacenter1 = LIFOSchedulingTaskScalable.createDatacenter("Datacenter_1")
    ...
    */
    ...
    val Some(broker: DatacenterBroker) = LIFOSchedulingTaskScalable.createBroker
    ...
    val vmlist = LIFOSchedulingTaskScalable.createVM(brokerId, total_vms) //creating 20 vms
    ...
    val cloudletList = LIFOSchedulingTaskScalable.createCloudlet(brokerId, total_cloudlets) // creating 40 cloudlets
    ...
    LIFOSchedulingTaskScalable.printCloudletList(newList)
    logger.info("Cloudlet results printed")

    Log.printLine("LIFOSchedulingTaskScalable completed!")
  } catch {
    case e: Exception =>
      e.printStackTrace()
  ...
  def createVM(userId: Int, vms: Int) = {
    ...
    range_vms.foreach((vm_no: Int) => {
      //assign virtual machine the LIFO scheduler.
      vm(vm_no) = new Vm(vm_no, userId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerLIFO)
      list += vm.apply(vm_no)
    })
    list
  }

...
import org.cloudsim.extensions.{CloudletWithCost}
 ...
 * In this simulation, use the cloudsim library's default space shared cloudlet scheduler.
 *
 * @author Amna Irfan
 */

// Main class
object SpaceSchedulingTaskScalable extends App with LazyLogging {

  /*
   * Load Typesafe Lightbend Configuration Files
   */
  val defaultConfig: Config = ConfigFactory.parseResources("simulation_space.conf")
  val fallbackConfig: Config = ConfigFactory.parseResources("simulation_space.conf")
    .withFallback(SpaceSchedulingTaskScalable.defaultConfig).resolve
  ...
  */
  Log.printLine("Starting Space-Shared Scheduling Task...")
  ...
  try {
    ...
    */
    ...
    @SuppressWarnings(Array.apply("unused")) val datacenter0 = SpaceSchedulingTaskScalable.createDatacenter("Datacenter_0")
    @SuppressWarnings(Array.apply("unused")) val datacenter1 = SpaceSchedulingTaskScalable.createDatacenter("Datacenter_1")
    ...
    */
    ...
    val Some(broker: DatacenterBroker) = SpaceSchedulingTaskScalable.createBroker
    ...
    val vmlist = SpaceSchedulingTaskScalable.createVM(brokerId, total_vms) //creating 20 vms
    ...
    val cloudletList = SpaceSchedulingTaskScalable.createCloudlet(brokerId, total_cloudlets) // creating 40 cloudlets
    ...
    SpaceSchedulingTaskScalable.printCloudletList(newList)
    logger.info("Cloudlet results printed")

    Log.printLine("SpaceSchedulingTaskScalable completed!")
  } catch {
    case e: Exception =>
      e.printStackTrace()
  ...
  def createVM(userId: Int, vms: Int) = {
    ...
    range_vms.foreach((vm_no: Int) => {
      //assign virtual machine the Space-shared scheduler.
      vm(vm_no) = new Vm(vm_no, userId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared)
      list += vm.apply(vm_no)
    })
    list
  }
   ...
   */
  def createDatacenter(name: String) : Option[Datacenter] = {
